...
export async function loginStep1Action(
  ...
  if (!user || !user.passwordHash) {
    // Hash a dummy value to keep response time roughly constant.
  ...
  // Trusted device → skip step 2 entirely.
...
export async function setupTotpAction(input: {
  ...
  await db.transaction(async (tx) => {
    ...
    // Replace any previous codes (paranoia — shouldn't exist on first enrollment).
  ...
  if (validated.data.trustDevice) {
    await issueTrustedDevice(user.id, user.organizationId);
  }
